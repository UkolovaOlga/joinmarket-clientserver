        ...
        if jm_coinjoin:
            ...
            while True:
                try:
                    ...
                    port += 1
        # (See above) For now these other two are just1K offsets from whatever we got,
        # or if the above was not executed, just 1K offsets from the initial port value.
        if snickerfactory:
            start_daemon(host, port-1000, sdfactory, usessl,
                                 './ssl/key.pem', './ssl/cert.pem')
            jlog.info("(SNICKER) Listening on port " + str(port-1000))
        if bip78:
            start_daemon(host, port-2000, bip78factory, usessl,
                                 './ssl/key.pem', './ssl/cert.pem')
            jlog.info("(BIP78) Listening on port " + str(port-2000))
